...
- `graph` emits a normalized `infra-agent.infra-graph` topology foundation
  ...
  AWS security group rules, IAM OIDC providers, AWS load balancing listener
  priorities, CloudFront aliases, API Gateway custom domains, Route53 records
  including ACM validation CNAMEs, and Kubernetes objects. Graph JSON includes
  compact `summary.impact` counts and the text output includes an `Impact`
  section for quick handoff. A stable graph snapshot fixture now covers the
...
- Runtime validation issue classification recognizes Pulumi provider
  ...
  conflicts, and reports them as non-repairable ordering blockers that require
