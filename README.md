...
- `graph` emits a normalized `infra-agent.infra-graph` topology foundation
  ...
  output includes an `Impact` section for quick handoff. A stable graph snapshot
  fixture now covers the cross-domain impact contract before any topology viewer
  work starts
