...
- `graph` emits a normalized `infra-agent.infra-graph` topology foundation
  ...
  AWS load balancing listener priorities, CloudFront aliases, API Gateway
  custom domains, Route53 records including ACM validation CNAMEs, and
  Kubernetes objects. Graph JSON includes compact `summary.impact` counts and
  the text output includes an `Impact` section for quick handoff. A stable
