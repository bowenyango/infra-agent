...
- `graph` emits a normalized `infra-agent.infra-graph` topology foundation
  ...
  `riskLevel`, `primaryConcern`, `recommendedAction`, and review-only
  `reviewSteps`, and the text output includes an `Impact` section for quick
  handoff. VPC-style security group rule matching keeps TCP/UDP ports required
  but permits omitted ports for
