...
async function writeValidInput(tempRoot) {
  ...
  const liveCheckBoundaryPath = join(tempRoot, 'knowledge-pack.upload-live-check-boundary.json');
  await writeFile(liveCheckBoundaryPath, `${JSON.stringify(liveCheckBoundary, null, 2)}\n`, 'utf8');
  return { liveCheckBoundary, liveCheckBoundaryPath };
}

test('knowledge upload-command-boundary command writes valid dry-run boundary JSON', async () => {
  const tempRoot = await mkdtemp(resolve(tmpdir(), 'infra-agent-upload-command-'));

  try {
    const { liveCheckBoundary, liveCheckBoundaryPath } = await writeValidInput(tempRoot);
    const boundaryPath = join(tempRoot, 'knowledge-pack.upload-command-boundary.json');
    const output = await captureStdout(() => main([
      'knowledge',
      'upload-command-boundary',
      liveCheckBoundaryPath,
      '--out',
      boundaryPath,
      '--json'
    ]));
    ...
    assertNoPrivateValues({ outputPayload, boundary }, liveCheckBoundary.target.objectKey);
  } finally {
    await rm(tempRoot, { recursive: true, force: true });
  }
});

test('knowledge upload-command-boundary command emits safe blocked text output', async () => {
  const tempRoot = await mkdtemp(resolve(tmpdir(), 'infra-agent-upload-command-text-'));

  try {
    const { liveCheckBoundary, liveCheckBoundaryPath } = await writeValidInput(tempRoot);
    await writeFile(liveCheckBoundaryPath, `${JSON.stringify({
      ...liveCheckBoundary,
      ...
      readiness: {
        ...
        blockers: [{
          ...
          path: '$.liveCheckBoundary.uploadCommandGenerated',
          message: 'upload command was generated'
        }]
      }
    }, null, 2)}\n`, 'utf8');
    ...
    assert.match(output, /Knowledge team upload command boundary/);
    ...
    assertNoPrivateValues(output, liveCheckBoundary.target.objectKey);
  } finally {
    await rm(tempRoot, { recursive: true, force: true });
  }
});
