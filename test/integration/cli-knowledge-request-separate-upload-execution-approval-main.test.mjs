...
async function writeValidInput(tempRoot) {
  ...
  const executionReadinessBoundaryPath = join(tempRoot, 'knowledge-pack.upload-execution-readiness-boundary.json');
  await writeFile(executionReadinessBoundaryPath, `${JSON.stringify(executionReadinessBoundary, null, 2)}\n`, 'utf8');
  return { executionReadinessBoundary, executionReadinessBoundaryPath };
}

test('knowledge request-separate-upload-execution-approval command writes valid dry-run request JSON', async () => {
  const tempRoot = await mkdtemp(resolve(tmpdir(), 'infra-agent-upload-execution-approval-request-'));

  try {
    const { executionReadinessBoundary, executionReadinessBoundaryPath } = await writeValidInput(tempRoot);
    const requestPath = join(tempRoot, 'knowledge-pack.upload-execution-approval-request.json');
    const output = await captureStdout(() => main([
      'knowledge',
      'request-separate-upload-execution-approval',
      executionReadinessBoundaryPath,
      '--out',
      requestPath,
      '--json'
    ]));
    ...
    const validation = validateKnowledgePayload(request, requestPath);

    assert.equal(outputPayload.outputPath, requestPath);
    assert.equal(request.outputPath, undefined);
    ...
    assert.equal(request.target.objectKey, undefined);
    ...
    assertNoPrivateValues({ outputPayload, request });
  } finally {
    await rm(tempRoot, { recursive: true, force: true });
  }
});

test('knowledge request-separate-upload-execution-approval command emits safe blocked text output', async () => {
  const tempRoot = await mkdtemp(resolve(tmpdir(), 'infra-agent-upload-execution-approval-request-text-'));

  try {
    const { executionReadinessBoundary, executionReadinessBoundaryPath } = await writeValidInput(tempRoot);
    await writeFile(executionReadinessBoundaryPath, `${JSON.stringify({
      ...
      readiness: {
        ...
        blockers: [{
          ...
          path: '$.remainingExecutionBoundaries.uploadExecutionApproved',
    ...
    assert.match(output, /Knowledge team upload execution approval request/);
