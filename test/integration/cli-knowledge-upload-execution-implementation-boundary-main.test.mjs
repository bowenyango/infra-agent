...
test('knowledge upload-execution-implementation-boundary command writes valid dry-run boundary JSON', async () => {
  const tempRoot = await mkdtemp(join(tmpdir(), 'infra-agent-implementation-boundary-'));
  try {
    const updateRecord = validPlanRulesUpdateRecord();
    const updateRecordPath = join(tempRoot, 'knowledge-pack.upload-execution-plan-rules-update-record.json');
    const boundaryPath = join(tempRoot, 'knowledge-pack.upload-execution-implementation-boundary.json');
    await writeFile(updateRecordPath, `${JSON.stringify(updateRecord, null, 2)}\n`, 'utf8');
    ...
    assert.equal(stdoutPayload.outputPath, boundaryPath);
    ...
    assert.equal(boundary.sourcePlanRulesUpdateRecord.policyUpdateAuthorized, false);
    ...
    assert.equal(validateKnowledgePayload(boundary, boundaryPath).valid, true);
...
test('knowledge upload-execution-implementation-boundary command emits safe blocked text output', async () => {
  const tempRoot = await mkdtemp(join(tmpdir(), 'infra-agent-implementation-boundary-'));
  try {
    const updateRecord = validPlanRulesUpdateRecord();
    const updateRecordPath = join(tempRoot, 'knowledge-pack.upload-execution-plan-rules-update-record.json');
    await writeFile(updateRecordPath, `${JSON.stringify({
      ...updateRecord,
    ...
    assert.match(output, /Knowledge team upload execution implementation boundary/);
