...
test('knowledge upload-execution-plan-rules-review command writes valid dry-run review JSON', async () => {
  const tempRoot = await mkdtemp(join(tmpdir(), 'infra-agent-plan-rules-review-'));
  try {
    const sourcePath = join(tempRoot, 'knowledge-pack.upload-execution-authorization-boundary.json');
    const reviewPath = join(tempRoot, 'knowledge-pack.upload-execution-plan-rules-review.json');
    await writeFile(sourcePath, `${JSON.stringify(validAuthorizationBoundary(), null, 2)}\n`, 'utf8');
...
test('knowledge upload-execution-plan-rules-review command emits safe blocked text output', async () => {
  const tempRoot = await mkdtemp(join(tmpdir(), 'infra-agent-plan-rules-review-'));
  try {
    const sourcePath = join(tempRoot, 'knowledge-pack.upload-execution-authorization-boundary.json');
    await writeFile(sourcePath, `${JSON.stringify({
      ...validAuthorizationBoundary(),
      uploadExecutionAllowed: true,
      target: {
        ...validAuthorizationBoundary().target,
      ...
      authorizationBoundary: {
        ...validAuthorizationBoundary().authorizationBoundary,
    ...
    assert.match(output, /Knowledge team upload execution plan\/rules review/);
