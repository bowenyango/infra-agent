...
async function buildValidArtifactBytesBoundary() {
...
async function writeValidInput(tempRoot) {
  const artifactBytesBoundary = await buildValidArtifactBytesBoundary();
  const artifactBytesBoundaryPath = join(tempRoot, 'knowledge-pack.upload-artifact-bytes-boundary.json');
  await writeFile(artifactBytesBoundaryPath, `${JSON.stringify(artifactBytesBoundary, null, 2)}\n`, 'utf8');
  return { artifactBytesBoundary, artifactBytesBoundaryPath };
}

test('knowledge upload-adapter-injection-boundary command writes valid dry-run boundary JSON', async () => {
  const tempRoot = await mkdtemp(resolve(tmpdir(), 'infra-agent-upload-adapter-injection-'));

  try {
    const { artifactBytesBoundary, artifactBytesBoundaryPath } = await writeValidInput(tempRoot);
    const boundaryPath = join(tempRoot, 'knowledge-pack.upload-adapter-injection-boundary.json');
    const output = await captureStdout(() => main([
      'knowledge',
      'upload-adapter-injection-boundary',
      artifactBytesBoundaryPath,
      '--out',
      boundaryPath,
      '--json'
    ]));
...
test('knowledge upload-adapter-injection-boundary command emits safe blocked text output', async () => {
  const tempRoot = await mkdtemp(resolve(tmpdir(), 'infra-agent-upload-adapter-injection-text-'));

  try {
    const { artifactBytesBoundary, artifactBytesBoundaryPath } = await writeValidInput(tempRoot);
    await writeFile(artifactBytesBoundaryPath, `${JSON.stringify({
    ...
    assert.match(output, /Knowledge team upload adapter injection boundary/);
