...
async function buildValidRollbackPlanBoundary() {
...
async function writeValidInput(tempRoot) {
  const rollbackPlanBoundary = await buildValidRollbackPlanBoundary();
  const rollbackPlanBoundaryPath = join(tempRoot, 'knowledge-pack.upload-rollback-plan-boundary.json');
  await writeFile(rollbackPlanBoundaryPath, `${JSON.stringify(rollbackPlanBoundary, null, 2)}\n`, 'utf8');
  return { rollbackPlanBoundary, rollbackPlanBoundaryPath };
}

test('knowledge upload-audit-record-boundary command writes valid dry-run boundary JSON', async () => {
  const tempRoot = await mkdtemp(resolve(tmpdir(), 'infra-agent-upload-audit-record-'));

  try {
    const { rollbackPlanBoundary, rollbackPlanBoundaryPath } = await writeValidInput(tempRoot);
    const boundaryPath = join(tempRoot, 'knowledge-pack.upload-audit-record-boundary.json');
    const output = await captureStdout(() => main([
      'knowledge',
      'upload-audit-record-boundary',
      rollbackPlanBoundaryPath,
      '--out',
      boundaryPath,
      '--json'
    ]));
...
test('knowledge upload-audit-record-boundary command emits safe blocked text output', async () => {
  const tempRoot = await mkdtemp(resolve(tmpdir(), 'infra-agent-upload-audit-record-text-'));

  try {
    const { rollbackPlanBoundary, rollbackPlanBoundaryPath } = await writeValidInput(tempRoot);
    await writeFile(rollbackPlanBoundaryPath, `${JSON.stringify({
    ...
    assert.match(output, /Knowledge team upload audit record boundary/);
