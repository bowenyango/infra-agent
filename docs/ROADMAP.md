...
1. Keep `inspect`, `inventory`, `pack`, `refs`, `cache status`, `knowledge sources/prefetch/extract/pack/index/validate`,
...
2. Evolve `inventory` so agents can read a compact list of tools,
   ...
   linkages without raw file content.
3. Evolve `changed` so git base/head impact can include Argo CD applications,
...
6. Evolve compact `refs` lookups for Terraform/Pulumi/Helm interfaces only
   when they are tied to repo usage or the requested scope. Public reference
   lookup is supporting context, not the product center.
...
| Inventory | Partial | `infra-agent inventory` emits a compact read-only `infra-agent.inventory` JSON/text report from existing inspection data. It summarizes detected Helm charts, Pulumi projects/stacks, Terraform roots, domain tools, environment hints, primary files, validation targets, semantic fact counts, knowledge-cache posture, compact Helm chart identity/version/dependency metadata, and filtered omitted target counts. | Argo CD/Kubernetes deployment linkage, Terraform module interface rollups, semantic-unit hash cache hit/miss reporting, and richer fact-kind summaries remain future work |
| Scoped pack | Partial | `infra-agent pack` emits compact Markdown or `infra-agent.scoped-pack` JSON from an explicit path, target name/id, environment hint, Pulumi stack, or `--changed` changed-component set. It selects matching inventory targets, suggested files, validation targets, environment hints, semantic fact counts, compact Helm chart metadata, compact changed-file/risk-hint summaries, and knowledge-cache posture without raw content or execution. | Argo CD/Kubernetes linkage, selected knowledge-unit enrichment, semantic cache hit/miss reuse, and graph-aware related systems remain future work |
| Refs | Partial | `infra-agent refs` emits a compact read-only `infra-agent.refs` JSON/text report from a required scope. It reuses scoped target resolution, repo-local config semantics, and selected knowledge source cache posture to return bounded interface facts and omissions without raw docs, source payloads, live fetch, validation, plan/preview, apply, deploy, or state mutation. | Field/resource-specific filters, unit-index-backed references, source-level unit counts, and richer Terraform/Pulumi/Helm public-reference enrichment remain future work |
