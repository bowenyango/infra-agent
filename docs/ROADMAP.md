...
- Graph summaries now include `edgesByKind` and compact `summary.impact`
  ...
  `recommendedAction`, `omittedReviewTargets`, and `mutationAllowed=false` for
  machine-readable handoff. Text graph output includes an `Impact` section with
  the most relevant rename, cascade, and ordering-conflict edges.
