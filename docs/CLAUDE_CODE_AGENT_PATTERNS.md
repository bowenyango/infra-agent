...
- Team knowledge publication and upload-boundary contracts are legacy
  `agent-1` scaffolding. They may be kept temporarily for compatibility while
  `agent-2` extracts the context-compiler core, but they must not guide new
  architecture, CLI design, tests, or skill instructions unless the user asks
  for a concrete team-upload product. New work should prefer read-only local
  cache, repo-curated packs, artifact validation, and deterministic metadata
  retrieval over any remote publication or upload workflow.
