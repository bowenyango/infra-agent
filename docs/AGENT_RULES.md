...
- Treat runtime `pulumi-create-before-delete-conflict` validation issues the same way, including DNS/domain families such as CloudFront aliases, API Gateway custom domains, and Route53 records, plus security group rules and IAM OIDC providers. They are blockers, not bounded automatic repairs.
- Some exclusive-identity specs intentionally use overlap matching, such as CloudFront aliases and security group rule source sets, or optional identity parts, such as Route53 `set_identifier`. Explain the matched identity keys from graph metadata and still confirm with native plan/preview/state before recommending DNS, alias, security group permission, or state changes.
