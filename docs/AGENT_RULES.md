...
- Treat graph `create-before-delete-conflict` edges as high-risk ordering warnings. For Pulumi AWS Routes, the exclusive identity is route table plus destination; remediation candidates are Pulumi aliases for logical renames, `deleteBeforeReplace` for true replacements with accepted downtime, or explicit state/import repair after human approval.
