...
- Compact `agent --json` output should expose runtime exclusive-identity blockers through `validation.identityConflicts` so downstream agents can consume engine, family, identity fields, source command, and suggested review action without parsing raw stderr or long guidance strings.
...
- Preserve `validation.identityConflicts[].reviewSteps` as a review-only checklist. It may suggest moved blocks, aliases, import/state repair, or sequencing analysis, but it must not trigger state or stack mutation without explicit approval and human-reviewed mappings.
