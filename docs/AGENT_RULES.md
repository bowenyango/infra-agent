...
- Treat runtime `pulumi-create-before-delete-conflict` validation issues the same way, including DNS/domain families such as CloudFront aliases, API Gateway custom domains, and Route53 records, plus listener rules, security group rules, and IAM OIDC providers. They are blockers, not bounded automatic repairs.
