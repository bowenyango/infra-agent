...
import { buildPublicKnowledgeLibraryFromUrl } from '../knowledge/public-library-build.ts';
import {
...
import { downloadPublicKnowledgeLibraryArtifact } from '../knowledge/public-library-download.ts';
import {
...
import { hasPublicKnowledgeLibrarySelectorFilter } from '../knowledge/public-library-select.ts';
import { buildPublicKnowledgeLibraryRefinementReviewReport } from '../knowledge/public-library-refinement-review.ts';
import { applyPublicKnowledgeLibraryRefinement } from '../knowledge/public-library-refinement-apply.ts';
import { runPublicKnowledgeLibraryRefinement } from '../knowledge/public-library-refinement-run.ts';
import {
...
import {
  ...
  printPublicKnowledgeLibraryDownloadReport,
  printPublicKnowledgeLibraryCatalogReport,
  printPublicKnowledgeLibraryBuildReport,
  printPublicKnowledgeLibraryRefinementReviewReport,
  printPublicKnowledgeLibraryRefinementApplyReport,
  printPublicKnowledgeLibraryRefinementRunReport,
...
export interface ParsedArgs {
  ...
  knowledgeAction?: 'sources' | 'prefetch' | 'extract' | 'validate' | 'pack' | 'index' | 'resource' | 'from-url' | 'publish' | 'library-from-url' | 'library-stage' | 'library-download' | 'library-catalog' | 'library-refinement-review' | 'library-refinement-run' | 'library-refinement-apply' | null;
...
export function parseArgs(argv: string[]): ParsedArgs {
  ...
  if (commandName === 'knowledge') {
    ...
    if (
      ...
      && knowledgeAction !== 'library-from-url'
      && knowledgeAction !== 'library-stage'
      && knowledgeAction !== 'library-download'
      && knowledgeAction !== 'library-catalog'
      && knowledgeAction !== 'library-refinement-review'
      && knowledgeAction !== 'library-refinement-run'
      && knowledgeAction !== 'library-refinement-apply'
    ) {
      fail('knowledge requires a supported action: sources, prefetch, extract, validate, pack, index, resource, from-url, publish, library-from-url, library-stage, library-download, library-catalog, library-refinement-review, library-refinement-run, or library-refinement-apply.');
    }
    ...
    for (let index = 0; index < actionArgs.length; index += 1) {
      ...
      if (arg === '--domain') {
        if (knowledgeAction === 'validate' || knowledgeAction === 'library-from-url' || knowledgeAction === 'library-stage' || knowledgeAction === 'library-refinement-review' || knowledgeAction === 'library-refinement-run' || knowledgeAction === 'library-refinement-apply') {
          fail('--domain is not supported for knowledge validate, knowledge library-from-url, knowledge library-stage, knowledge library-refinement-review, knowledge library-refinement-run, or knowledge library-refinement-apply.');
        }
      ...
      if (arg === '--target') {
        if (knowledgeAction === 'validate' || knowledgeAction === 'library-from-url' || knowledgeAction === 'library-stage' || knowledgeAction === 'library-download' || knowledgeAction === 'library-catalog' || knowledgeAction === 'library-refinement-review' || knowledgeAction === 'library-refinement-run' || knowledgeAction === 'library-refinement-apply') {
          fail('--target is not supported for knowledge validate, knowledge library-from-url, knowledge library-stage, knowledge library-download, knowledge library-catalog, knowledge library-refinement-review, knowledge library-refinement-run, or knowledge library-refinement-apply.');
        }
        if (knowledgeAction === 'from-url') {
      ...
      if (arg === '--resource') {
        ...
        if (knowledgeAction === 'validate' || knowledgeAction === 'publish' || knowledgeAction === 'from-url' || knowledgeAction === 'library-from-url' || knowledgeAction === 'library-stage' || knowledgeAction === 'library-refinement-review' || knowledgeAction === 'library-refinement-run' || knowledgeAction === 'library-refinement-apply') {
          fail('--resource is not supported for knowledge validate, knowledge from-url, knowledge library-from-url, knowledge publish, knowledge library-stage, knowledge library-refinement-review, knowledge library-refinement-run, or knowledge library-refinement-apply.');
        }
      ...
      if (arg === '--provider') {
        ...
        if (knowledgeAction === 'library-catalog' || knowledgeAction === 'library-download') {
        ...
        if (knowledgeAction !== 'index') {
          fail('--provider is only supported for knowledge index, knowledge publish, knowledge library-catalog, or knowledge library-download.');
        }
      ...
      if (arg === '--package') {
        ...
        if (knowledgeAction !== 'index') {
          fail('--package is only supported for knowledge index, knowledge publish, knowledge library-catalog, or knowledge library-download.');
        }
      ...
      if (arg === '--chart') {
        ...
        if (knowledgeAction !== 'index') {
          fail('--chart is only supported for knowledge index, knowledge publish, knowledge library-catalog, or knowledge library-download.');
        }
      ...
      if (arg === '--version') {
        ...
        if (knowledgeAction !== 'index') {
          fail('--version is only supported for knowledge index, knowledge publish, knowledge library-catalog, or knowledge library-download.');
        }
      ...
      if (arg === '--out') {
        ...
        if (
          ...
          fail('--out is only supported for knowledge extract, knowledge pack, knowledge index, knowledge resource, knowledge from-url, knowledge publish, knowledge library-from-url, knowledge library-stage, knowledge library-download, knowledge library-catalog, knowledge library-refinement-review, knowledge library-refinement-run, or knowledge library-refinement-apply.');
        }
        if (outputPath !== null) {
      ...
      if (arg === '--library-out') {
        ...
        if (knowledgeAction !== 'from-url' && knowledgeAction !== 'library-from-url') {
          fail('--library-out is only supported for knowledge from-url or knowledge library-from-url.');
        }
      ...
      if (arg === '--refined') {
        const refinedValue = actionArgs[index + 1]?.trim();
        if (!refinedValue) {
          fail('Missing value for --refined.');
        }
        if (knowledgeAction !== 'library-refinement-apply') {
          fail('--refined is only supported for knowledge library-refinement-apply.');
        }
        if (publicLibraryRefinedPath !== null) {
          fail('--refined can be provided at most once.');
        }

        publicLibraryRefinedPath = refinedValue;
        index += 1;
        continue;
      }
      ...
      if (arg === '--refined-out') {
        const refinedValue = actionArgs[index + 1]?.trim();
        if (!refinedValue) {
          fail('Missing value for --refined-out.');
        }
        if (knowledgeAction !== 'library-from-url') {
          fail('--refined-out is only supported for knowledge library-from-url.');
        }
        if (publicLibraryRefinedOutputPath !== null) {
          fail('--refined-out can be provided at most once.');
        }

        publicLibraryRefinedOutputPath = refinedValue;
        index += 1;
        continue;
      }
      ...
      if (arg === '--refine') {
        ...
        publicLibraryFromUrlRefine = true;
        continue;
      }

      if (arg === '--model' || arg === '--llm-model') {
        const modelValue = actionArgs[index + 1]?.trim();
        if (!modelValue) {
          fail(`Missing value for ${arg}.`);
        }
        if (knowledgeAction !== 'library-refinement-run' && knowledgeAction !== 'library-from-url') {
          fail(`${arg} is only supported for knowledge library-refinement-run or knowledge library-from-url.`);
        }
        if (llmModel !== null) {
          fail('LLM model can be provided at most once.');
        }
      ...
      if (arg === '--openai-base-url' || arg === '--llm-base-url') {
        const baseUrlValue = actionArgs[index + 1]?.trim();
        if (!baseUrlValue) {
          fail(`Missing value for ${arg}.`);
        }
        if (knowledgeAction !== 'library-refinement-run' && knowledgeAction !== 'library-from-url') {
      ...
      if (arg === '--llm-provider') {
        const providerValue = actionArgs[index + 1];
        if (providerValue !== 'openai-compatible') {
          fail('Missing or invalid value for --llm-provider. Expected openai-compatible.');
        }
        if (knowledgeAction !== 'library-refinement-run' && knowledgeAction !== 'library-from-url') {
          fail('--llm-provider is only supported for knowledge library-refinement-run or knowledge library-from-url.');
        }
        if (llmProvider !== null) {
          fail('LLM provider can be provided at most once.');
        }
      ...
      if (arg === '--content') {
        ...
        if (knowledgeAction !== 'from-url' && knowledgeAction !== 'library-from-url') {
          fail('--content is only supported for knowledge from-url or knowledge library-from-url.');
        }
        if (contentPath !== null) {
      ...
      if (arg === '--max-units') {
        ...
        if (knowledgeAction !== 'pack' && knowledgeAction !== 'index' && knowledgeAction !== 'resource' && knowledgeAction !== 'from-url' && knowledgeAction !== 'library-from-url') {
          fail('--max-units is only supported for knowledge pack, knowledge index, knowledge resource, knowledge from-url, or knowledge library-from-url.');
        }
      ...
      if (arg === '--workspace') {
        if (knowledgeAction !== 'validate' && knowledgeAction !== 'publish' && knowledgeAction !== 'library-from-url' && knowledgeAction !== 'library-stage' && knowledgeAction !== 'library-download') {
          fail('--workspace is only supported for knowledge validate, knowledge publish, knowledge library-from-url, knowledge library-stage, or knowledge library-download.');
        }
        ...
        if (knowledgeAction === 'publish' || knowledgeAction === 'library-from-url' || knowledgeAction === 'library-stage' || knowledgeAction === 'library-download') {
      ...
      if (arg === '--store-dir') {
        if (knowledgeAction !== 'publish' && knowledgeAction !== 'library-from-url' && knowledgeAction !== 'library-stage' && knowledgeAction !== 'library-download') {
          fail('--store-dir is only supported for knowledge publish, knowledge library-from-url, knowledge library-stage, or knowledge library-download.');
        }
      ...
      if (arg === '--registry') {
        if (knowledgeAction !== 'publish' && knowledgeAction !== 'library-from-url' && knowledgeAction !== 'library-stage') {
          fail('--registry is only supported for knowledge publish, knowledge library-from-url, or knowledge library-stage.');
        }
      ...
      if (arg === '--tag') {
        ...
        if (knowledgeAction !== 'library-catalog' && knowledgeAction !== 'library-download') {
          fail('--tag is only supported for knowledge library-catalog or knowledge library-download.');
        }
      ...
      if (arg === '--quality') {
        ...
        if (knowledgeAction !== 'library-catalog' && knowledgeAction !== 'library-download') {
          fail('--quality is only supported for knowledge library-catalog or knowledge library-download.');
        }
      ...
      if (arg === '--coordinate' || arg === '--coordinates') {
        ...
        if (knowledgeAction !== 'library-catalog' && knowledgeAction !== 'library-download') {
          fail(`${arg} is only supported for knowledge library-catalog or knowledge library-download.`);
        }
        if (knowledgeAction === 'library-download') {
          if (publicLibraryDownloadCoordinate !== null) {
            fail('--coordinate can be provided at most once.');
          }
          publicLibraryDownloadCoordinate = coordinates;
          index += 1;
          continue;
        }
    ...
    if (knowledgeAction !== 'from-url' && knowledgeAction !== 'library-from-url' && positionalArgs.length > 1) {
    ...
    if (knowledgeAction === 'library-from-url' && positionalArgs.length !== 1) {
      fail('knowledge library-from-url requires exactly one public documentation URL.');
    }
    ...
    if (knowledgeAction === 'library-from-url' && libraryOutputPath === null) {
      fail('knowledge library-from-url requires --library-out.');
    }
    if (knowledgeAction === 'library-from-url' && publicLibraryFromUrlRefine && publicLibraryRefinedOutputPath === null) {
      fail('knowledge library-from-url --refine requires --refined-out.');
    }
    if (knowledgeAction === 'library-from-url' && !publicLibraryFromUrlRefine && publicLibraryRefinedOutputPath !== null) {
      fail('knowledge library-from-url --refined-out requires --refine.');
    }
    if (knowledgeAction === 'library-from-url' && !publicLibraryFromUrlRefine && (llmProvider !== null || llmModel !== null || llmBaseUrl !== null)) {
      fail('knowledge library-from-url LLM flags require --refine.');
    }
    if (knowledgeAction === 'library-from-url') {
      const stageFlagCount = [
        stageWorkspaceProvided,
        publishStoreDir !== null,
        publishRegistryPath !== null
      ].filter(Boolean).length;
      if (stageFlagCount > 0 && stageFlagCount < 3) {
        fail('knowledge library-from-url staging requires --workspace, --store-dir, and --registry together.');
      }
    }
    ...
    if (knowledgeAction === 'library-refinement-run' && positionalArgs.length !== 1) {
      fail('knowledge library-refinement-run requires exactly one public library artifact JSON path.');
    }
    if (knowledgeAction === 'library-refinement-apply' && positionalArgs.length !== 1) {
      fail('knowledge library-refinement-apply requires exactly one public library artifact JSON path.');
    }
    if (knowledgeAction === 'library-refinement-run' && outputPath === null) {
      fail('knowledge library-refinement-run requires --out.');
    }
    if (knowledgeAction === 'library-refinement-apply' && publicLibraryRefinedPath === null) {
      fail('knowledge library-refinement-apply requires --refined.');
    }
    if (knowledgeAction === 'library-refinement-apply' && outputPath === null) {
      fail('knowledge library-refinement-apply requires --out.');
    }
    ...
    if (knowledgeAction === 'library-download' && !stageWorkspaceProvided) {
      fail('knowledge library-download requires --workspace.');
    }
    ...
    if (knowledgeAction === 'library-download') {
      const hasSelectorFilter = hasPublicKnowledgeLibrarySelectorFilter(publicLibraryCatalogFilter);
      if (publicLibraryDownloadCoordinate !== null && hasSelectorFilter) {
        fail('knowledge library-download accepts either --coordinate or selector filters, not both.');
      }
      if (publicLibraryDownloadCoordinate === null && !hasSelectorFilter) {
        fail('knowledge library-download requires --coordinate or at least one selector filter.');
      }
    }
    ...
    if (knowledgeAction !== 'validate' && knowledgeAction !== 'publish' && knowledgeAction !== 'from-url' && knowledgeAction !== 'library-from-url' && knowledgeAction !== 'library-stage' && knowledgeAction !== 'library-download' && knowledgeAction !== 'library-catalog' && knowledgeAction !== 'library-refinement-review' && knowledgeAction !== 'library-refinement-run' && knowledgeAction !== 'library-refinement-apply') {
      workspace = positionalArgs[0] ?? workspace;
    }
    ...
    return {
      ...
      inputPath: knowledgeAction === 'validate' || knowledgeAction === 'publish' || knowledgeAction === 'from-url' || knowledgeAction === 'library-from-url' || knowledgeAction === 'library-stage' || knowledgeAction === 'library-download' || knowledgeAction === 'library-catalog' || knowledgeAction === 'library-refinement-review' || knowledgeAction === 'library-refinement-run' || knowledgeAction === 'library-refinement-apply'
        ? positionalArgs[0]
        : null,
      outputPath,
      libraryOutputPath,
...
export async function main(argv = process.argv.slice(2)): Promise<void> {
  ...
  if (parsed.command === 'knowledge' && parsed.knowledgeAction === 'library-from-url') {
    if (!parsed.inputPath || !parsed.libraryOutputPath) {
      fail('knowledge library-from-url requires a public documentation URL and --library-out.');
    }

    const llmConfig = parsed.publicLibraryFromUrlRefine
      ? resolveLLMClientConfig(process.env, buildLLMClientConfigOverrides(parsed))
      : null;
    if (parsed.publicLibraryFromUrlRefine && !llmConfig) {
      fail('knowledge library-from-url --refine requires an LLM API key. Set INFRA_AGENT_OPENAI_API_KEY or OPENAI_API_KEY.');
    }
    ...
    const libraryWrittenPath = await writeJsonArtifact(parsed.libraryOutputPath, cwd(), result.artifact);
    const refinedWrittenPath = parsed.publicLibraryFromUrlRefine && parsed.publicLibraryRefinedOutputPath
      ? await writeJsonArtifact(parsed.publicLibraryRefinedOutputPath, cwd(), result.urlReport)
      : null;
    const stageReport = parsed.publishStoreDir && parsed.publishRegistryPath
      ? await stagePublicKnowledgeLibraryArtifact({
          workspaceRoot: parsed.workspace,
          artifactPath: libraryWrittenPath,
    ...
    const reportPayload = {
      ...
      libraryOutputPath: libraryWrittenPath,
      ...(refinedWrittenPath ? { refinedOutputPath: refinedWrittenPath } : {}),
      ...(stageReport ? { stage: stageReport } : {})
    };
    const writtenPath = parsed.outputPath
    ...
    printPublicKnowledgeLibraryBuildReport(reportPayload);
    if (writtenPath) {
      process.stdout.write(`\nwritten: ${writtenPath}\n`);
    }
    process.stdout.write(`library artifact: ${libraryWrittenPath}\n`);
    if (refinedWrittenPath) {
      process.stdout.write(`refined report: ${refinedWrittenPath}\n`);
    }
    if (stageReport) {
      process.stdout.write(`registry: ${stageReport.registry.path}\n`);
    }
    return;
  }
  ...
  if (parsed.command === 'knowledge' && parsed.knowledgeAction === 'library-refinement-review') {
    if (!parsed.inputPath) {
      fail('knowledge library-refinement-review requires exactly one public library artifact JSON path.');
    }
    ...
    if (validationReport.inputKind !== 'infra-agent.public-knowledge-library-artifact') {
      fail('knowledge library-refinement-review requires an infra-agent.public-knowledge-library-artifact input.');
    }

    const report = await buildPublicKnowledgeLibraryRefinementReviewReport({
    ...
    printPublicKnowledgeLibraryRefinementReviewReport(report);
    if (writtenPath) {
      process.stdout.write(`\nwritten: ${writtenPath}\n`);
    }
    return;
  ...
  if (parsed.command === 'knowledge' && parsed.knowledgeAction === 'library-refinement-run') {
    if (!parsed.inputPath || !parsed.outputPath) {
      fail('knowledge library-refinement-run requires a public library artifact JSON path and --out.');
    }
    ...
    if (validationReport.inputKind !== 'infra-agent.public-knowledge-library-artifact') {
      fail('knowledge library-refinement-run requires an infra-agent.public-knowledge-library-artifact input.');
    }

    const llmConfig = resolveLLMClientConfig(process.env, buildLLMClientConfigOverrides(parsed));
    if (!llmConfig) {
      fail('knowledge library-refinement-run requires an LLM API key. Set INFRA_AGENT_OPENAI_API_KEY or OPENAI_API_KEY.');
    }
    ...
    const writtenPath = await writeJsonArtifact(parsed.outputPath, cwd(), refinedReport);
    const reportPayload = {
      ...report,
      outputPath: writtenPath
    };
    ...
    printPublicKnowledgeLibraryRefinementRunReport(reportPayload);
    process.stdout.write(`\nrefined report: ${writtenPath}\n`);
    return;
  }

  if (parsed.command === 'knowledge' && parsed.knowledgeAction === 'library-refinement-apply') {
    if (!parsed.inputPath || !parsed.publicLibraryRefinedPath || !parsed.outputPath) {
      fail('knowledge library-refinement-apply requires a public library artifact JSON path, --refined, and --out.');
    }

    const artifactValidationReport = await loadKnowledgeValidationReport(parsed.inputPath, cwd(), {});
    if (!artifactValidationReport.valid) {
      if (parsed.json) {
        process.stdout.write(`${JSON.stringify(artifactValidationReport, null, 2)}\n`);
      } else {
        printKnowledgeValidationReport(artifactValidationReport);
      }
      process.exitCode = 1;
      return;
    }
    if (artifactValidationReport.inputKind !== 'infra-agent.public-knowledge-library-artifact') {
      fail('knowledge library-refinement-apply requires an infra-agent.public-knowledge-library-artifact input.');
    }

    const refinedValidationReport = await loadKnowledgeValidationReport(parsed.publicLibraryRefinedPath, cwd(), {});
    if (!refinedValidationReport.valid) {
      if (parsed.json) {
        process.stdout.write(`${JSON.stringify(refinedValidationReport, null, 2)}\n`);
      } else {
        printKnowledgeValidationReport(refinedValidationReport);
      }
      process.exitCode = 1;
      return;
    }
    if (refinedValidationReport.inputKind !== 'infra-agent.public-knowledge-url-report') {
      fail('knowledge library-refinement-apply --refined requires an infra-agent.public-knowledge-url-report input.');
    }

    const { report, artifact } = await applyPublicKnowledgeLibraryRefinement({
      artifactPath: parsed.inputPath,
      refinedReportPath: parsed.publicLibraryRefinedPath,
      baseDir: cwd()
    });
    const writtenPath = await writeJsonArtifact(parsed.outputPath, cwd(), artifact);
    const reportPayload = {
      ...report,
      outputPath: writtenPath
    };
    ...
    printPublicKnowledgeLibraryRefinementApplyReport(reportPayload);
    process.stdout.write(`\nupdated artifact: ${writtenPath}\n`);
    return;
  }
  ...
  if (parsed.command === 'knowledge' && parsed.knowledgeAction === 'library-download') {
    if (!parsed.inputPath || !parsed.publishStoreDir) {
      fail('knowledge library-download requires a public library registry JSON path or URL, --workspace, --store-dir, and --coordinate or selector filters.');
    }
    ...
    const report = await downloadPublicKnowledgeLibraryArtifact({
      ...
      registryPath: parsed.inputPath,
      storeDir: parsed.publishStoreDir,
      ...(parsed.publicLibraryDownloadCoordinate ? { coordinates: parsed.publicLibraryDownloadCoordinate } : {}),
      ...(parsed.publicLibraryCatalogFilter ? { selector: parsed.publicLibraryCatalogFilter } : {})
    });
